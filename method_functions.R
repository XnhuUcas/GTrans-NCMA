...
DataGenerate.A.size.0 <- function(K=10, h, n.vec, s=10, p=100, type='Toep'){
  ...
  if(min(eigen(Sig)$values)<0.05){
    Sig <- Sig + diag(0.1-min(eigen(Sig)$values),p)
  }
  ...
  for(k in 1:K){
    ...
    Omega.vec <- c(
      Omega.vec,
      max(colSums(abs(diag(1,p)-Sig.k%*%Theta)))
    )
  }
